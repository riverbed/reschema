...
class SchemaRule(RuleBase):
    ...
    def __call__(self, schema, validator):
        """
        Run the rule
        ...
        return self._rule_func(schema, validator)
...
class Validator(object):
    ...
    def _run_rules(self, rules, obj, validator=None):
        """
        ...
        for rule in rules:
            try:
                ret = None
                ret = rule(obj) if validator is None else rule(obj, validator)
            ...
            if ret is None:
                ...
                # ret is a list of results, generated by a schema rule
                results.extend(ret)
...
def validate_schema(schema, validator, results):
    """
    run type rule and schema rule against a schema and populate results
    """
    results.extend(validator._run_rules(validator.TYPE_RULES, schema))
    results.extend(validator._run_rules(validator.SCHEMA_RULES,
                                        schema, validator))
...
@Validator.schema('S0001')
def check_valid_schema(schema, validator):
    """
    ...
    if hasattr(schema, 'properties'):
        for _, prop in schema.properties.iteritems():
            validate_schema(prop, validator, results)

    if hasattr(schema, 'items'):
        validate_schema(schema.items, validator, results)

    if hasattr(schema, 'request') and schema.request is not None:
        validate_schema(schema.request, validator, results)

    if hasattr(schema, 'response') and schema.request is not None:
        validate_schema(schema.response, validator, results)

    if hasattr(schema, 'anyof'):
        for subschema in schema.anyof:
            validate_schema(subschema, validator, results)
    ...
    if hasattr(schema, 'not_') and schema.not_ is not None:
        validate_schema(schema.not_, validator, results)

    if hasattr(schema, '_params'):
        for _, param in schema._params.iteritems():
            validate_schema(param, validator, results)

    return results


@Validator.schema('S0002')
def required_is_valid(schema, validator):
